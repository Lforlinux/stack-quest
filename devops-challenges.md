# StackQuest DevOps Challenges 🚀

A collection of challenging DevOps scenarios and exercises to test and improve your skills.
...
A company wants to roll out a web service built on Kubernetes. To make this happen, complete a file stub located at /home/ubuntu/stackquest-q1/special-definition.yml with the steps that do the following:
...
- The finalized solution will be assessed in a fresh, isolated environment. Make sure all the configurations are stored in the /home/ubuntu/stackquest directory.
...
In the directory `/home/ubuntu/stackquest-q4/taskrunner.sh`, you have an SSL certificate named `security.crt` and a private key named `secrecy.key`. Complete the `taskrunner.sh` file with steps to accomplish the following:

- **Task 1**: Create a new User entry in Kubeconfig named `operator` using the `security.crt` and `secrecy.key` located in `/home/ubuntu/stackquest-q4/`.
  
- **Task 2**: Create a new Context entry named `operator` in Kubeconfig for the newly created User `operator`.

**Notes**

- The completed solution will be evaluated in a fresh environment. Make sure all your changes are in the `taskrunner.sh` file located in `/home/ubuntu/stackquest-q4/`. Manual changes will not be preserved.
...
You are required to complete the definition file located at `/home/ubuntu/stackquest-q5/cluster-role-def.yml`. The file should have configurations to perform the following actions:
...
- The completed solution will be verified in a fresh, untouched environment. Ensure all your changes are confined to the `cluster-role-def.yml` file in `/home/ubuntu/stackquest-q5`.
...
- Your Terraform script should be saved as `ec2_instance.tf` in the directory `/home/ubuntu/terraform-stackquest`.
...
- Save your Ansible playbook in a file named `web_server_setup.yml` in the directory `/home/ubuntu/stackquest-ansible`.
